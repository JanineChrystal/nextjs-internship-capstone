...
describe("toUserFacingError", () => {
	it("returns the generic message when the thrown value is not an Error", () => {
		expect(toUserFacingError("a string", ALLOWED)).toBe(
			"An unexpected error occurred",
		);
		expect(toUserFacingError(null, ALLOWED)).toBe(
			"An unexpected error occurred",
		);
	...
	it("passes through an allowed message unchanged", () => {
		expect(toUserFacingError(new Error("Not found"), ALLOWED)).toBe(
			"Not found",
		);
		expect(toUserFacingError(new Error("Unauthorized"), ALLOWED)).toBe(
			"Unauthorized",
		);
	});
	...
	it("applies an override when the message is in the override map", () => {
		...
		expect(
			toUserFacingError(new Error("Unauthorized"), ALLOWED, overrides),
