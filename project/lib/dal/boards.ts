...
export async function createBoardDAL(projectId: string, workspaceId: string, name: string) {
	const user = await getCurrentUser();
	if (!user) throw new Error("Unauthorized");
	...
	try {
		...
		const maxPosition = existingBoards.length > 0 ? Math.max(...existingBoards.map(b => b.position)) : -1;
		...
		await db.transaction(async (tx) => {
			for (let i = 0; i < boardIds.length; i++) {
				await tx.update(boards)
					...
					.where(and(eq(boards.id, boardIds[i]), eq(boards.projectId, projectId)));
			}
		});
		return true;
	} catch (error) {
